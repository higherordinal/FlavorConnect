...
/**
 * Generates a URL using a named route
 * 
 * This function provides a bridge between the traditional url_for() approach
 * and the router's named routes. It allows gradual migration to named routes
 * without breaking existing code.
 * 
 * @param string $route_name The name of the route as defined in routes.php
 * @param array $params Route parameters (for route placeholders)
 * @param array $query_params Query string parameters
 * @return string The generated URL
 */
function route($route_name, $params = [], $query_params = []) {
    global $router;
    
    if (!isset($router)) {
        // Fallback to traditional URL if router is not available
        $url = '';
        
        // Map common route names to their traditional URLs
        switch ($route_name) {
            case 'home':
                $url = '/index.php';
                break;
            case 'recipes.index':
                ...
                break;
            case 'recipes.show':
                $url = '/recipes/show.php';
                break;
            // Add more mappings as needed
            default:
                // For unknown routes, try to guess based on the route name
                $parts = explode('.', $route_name);
                $url = '/' . implode('/', $parts) . '.php';
        }
        ...
        if (!empty($query_params) && empty($params)) {
    ...
    try {
        // Generate URL using the router
        $url = $router->url($route_name, $params);
        ...
        // Log the error and fall back to traditional URL
        error_log("Error generating route for {$route_name}: " . $e->getMessage());
        return url_for('/');
    }
}
